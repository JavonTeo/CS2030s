...
class BusAPI {
   ...
   *     query fails, wrapped in a CompletableFuture.
   */
  private static CompletableFuture<HttpResponse<String>> httpGet(String url) {
    ...
    return client.sendAsync(request, BodyHandlers.ofString()); // TODO 
  }
   ...
   *     the bus stops that are serviced at this bus, or an empty
   *     string if something go wrong, wrapped in a CompletableFuture.
   */ 
  public static CompletableFuture<String> getBusStopsServedBy(String serviceId) {
    return httpGet(BUS_SERVICE_API + serviceId)
      .thenApplyAsync(x -> x.body());
  }
   ...
   *     string if the API query failed, wrapped in a CompletableFuture.
   */ 
